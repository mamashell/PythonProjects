...
WIDTH, HEIGHT = 690, 750

SCREEN = pygame.display.set_mode((WIDTH, HEIGHT), pygame.RESIZABLE)
# PIC = pygame.Surface((150, 100), pygame.SRCALPHA)
# PIC.fill(pygame.color.Color("white"))
# CENTER_POS = (WIDTH // 2, HEIGHT // 2)
BACKGROUND = pygame.image.load("assets/Starting Tiles.png")
BACKGROUND_RECT = BACKGROUND.get_rect(center=(317, 300))
ICON = pygame.image.load("assets/Icon.png")

pygame.display.set_caption("Jamaican Wordle 🌴!")
...
CORRECT_WORD = random.choice(JAMAICAN_WORD_LIST)
...
def check_guess(guess_to_check):
    ...
    for i in range(5):
        ...
        if lowercase_letter in CORRECT_WORD:
            if lowercase_letter == CORRECT_WORD[i]: #and lowercase_letter not in guessed_letters:
                ...
                #correct_positions.add(lowercase_letter)
                #guessed_letters.add(lowercase_letter)
                if not game_decided:
                    game_result = "W"
            else: #lowercase_letter not in correct_positions:
...
def display_error_message(message):
    # Display an error message on the screen if an invalid word is entered.
    error_font = pygame.font.Font("assets/FreeSansBold.otf", 25)
    error_text = error_font.render(message, True, "darkred")
    error_rect = error_text.get_rect(center=(WIDTH/2, 50))
